...
function EditableItem({ item, storeIdx, onUpdate, onRemove, theme, index, priceHistory, onPriceCommit }) {
  const [editingName, setEditingName] = useState(false);
  const [nameText, setNameText] = useState(item.name);
  const [qtyText, setQtyText] = useState(String(item.quantity));

  const historyPrice = priceHistory
    ? (priceHistory[item.name?.toLowerCase().trim()] ?? 0)
    : 0;
  const initialPrice = item.unitPrice > 0
    ? item.unitPrice.toFixed(2)
    : (historyPrice > 0 ? historyPrice.toFixed(2) : '');
  ...
  const commitPrice = (val) => {
    ...
    if (price > 0 && onPriceCommit) onPriceCommit(item.name, price);
  };
      ...
      <View style={styles.itemMain}>
        ...
        <View style={styles.fieldsRow}>
          ...
          <View style={{ gap: 3 }}>
            ...
            {historyPrice > 0 && item.unitPrice === 0 && (
...
export default function ReviewScreen({ navigation, route }) {
  ...
  const { loadList, saveToHistory, priceHistory, updatePriceHistory } = useShopping();
  ...
  const ListHeader = (
    <Animated.View entering={FadeInDown.duration(300).springify().damping(20)}>
        ...
        <Image source={require('../../assets/MealKet-Icon.png')} style={{ width: 24, height: 24, borderRadius: 6 }} />
        <View style={{ flex: 1 }}>
            ...
            >
              ...
              {storeData.items.map((product, idx) => (
                <EditableItem
                  ...
                  onPriceCommit={updatePriceHistory}
                />
              ))}
            </Animated.View>
          );
...
const styles = StyleSheet.create({
  ...
  priceHint: { fontSize: 10, fontWeight: '500', letterSpacing: 0.2 },
